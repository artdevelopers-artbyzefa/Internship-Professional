...
export default defineConfig(({ command }) => {
  ...
  return {
    plugins: [
      react(),
      tailwindcss(),
      // Gzip & Brotli compression only for the final production build (skips in watch mode)
      !isWatch && compression({ algorithm: 'brotliCompress', ext: '.br', threshold: 1024 }),
      !isWatch && compression({ algorithm: 'gzip', ext: '.gz', threshold: 1024 }),
      // Bundle visualizer only for the non-watch production build
      !isWatch && visualizer({ open: false, filename: 'bundle-stats.html', gzipSize: true, brotliSize: true }),
    ].filter(Boolean),
    build: {
